...
/**
 * Every thread of this JVM in JSON: state, stack, the locks it holds and the one it waits for, so a deadlock
 * is visible in the answer. It stops the world for as long as the dump takes and runs on the event loop of
 * the request - an operator's endpoint, and not one to leave open to the world.
 */
public class JsonJvmThreadDump implements JsonRestHandle {
