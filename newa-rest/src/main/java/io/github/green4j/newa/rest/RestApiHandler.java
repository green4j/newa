...
/**
 * The last handler of a REST pipeline: resolves every request against the {@link RestRouter} it was built
 * with, runs the handle which answers it, and renders what went wrong through the {@link HttpErrorHandler}.
 * <p>
 * It answers everything it sees - a request nothing routed becomes a {@code 404} - so nothing placed behind
 * it would ever run. A filter, a file handler or anything else which may decline a request goes in front.
 * <p>
 * It expects a decoded and aggregated request, so it stands behind {@code HttpServerCodec} and
 * {@code HttpObjectAggregator}, and it is not {@code @Sharable}: one per channel.
 */
