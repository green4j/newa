...
/**
 * One route as it was registered, and what the help endpoint reads to describe it: the path expression, the
 * handle behind it, and the descriptions a builder chained on.
 * <p>
 * {@code withPathParameterDescriptions} is not decoration - a path expression carrying {@code {name}}
 * requires one description per parameter, and the count is checked when the api is built.
 */
